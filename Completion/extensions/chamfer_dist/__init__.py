...
class ChamferDistanceL2_var(torch.nn.Module):
    f''' Chamder Distance L2
    '''
    def __init__(self, ignore_zeros=False):
        super().__init__()
    ...
    def forward(self, xyz1, xyz2, logvar):
        batch_size = xyz1.size(0)
        if batch_size == 1 and self.ignore_zeros:
            non_zeros1 = torch.sum(xyz1, dim=2).ne(0)
            non_zeros2 = torch.sum(xyz2, dim=2).ne(0)
        ...
        dist1, dist2 = ChamferFunction.apply(xyz1, xyz2)
        distance = ((0.5 * var * dist1[:, :8192] + dist2[:, :8192]) + (0.5 * logvar)) / 8192
        # distance = var * dist1[:,:6144] + logvar
        distance=distance.mean()
        # print('dist', distance)
        return distance #+ (torch.mean(dist1) + torch.mean(dist2))
        #return torch.mean(dist1) + torch.mean(dist2)
