...
class Kinova_LfD:
    ...
    def check_gripper_pose(self, actual_pose):
        self.last_action_notif_type = None
        # Get the actual cartesian pose to increment it
        feedback = rospy.wait_for_message("/" + self.robot_name + "/base_feedback", BaseCyclic_Feedback)
        
        actual_pose.position.x = feedback.base.commanded_tool_pose_x
        actual_pose.position.y = feedback.base.commanded_tool_pose_y
        actual_pose.position.z = feedback.base.commanded_tool_pose_z
        actual_pose.orientation.x = feedback.base.commanded_tool_pose_theta_x
        actual_pose.orientation.y = feedback.base.commanded_tool_pose_theta_y
        actual_pose.orientation.z = feedback.base.commanded_tool_pose_theta_z
        
        return actual_pose

    def example_send_cartesian_pose(self, actual_pose):
        self.last_action_notif_type = None
        # Get the actual cartesian pose to increment it
    ...
    def main(self, actual_pose):
        # For testing purposes
        success = self.is_init_success
        try:
            rospy.delete_param("/kortex_examples_test_results/approach_object_movement")
        except:
            pass
        ...
        if success:
            ...
            success &= self.example_send_cartesian_pose(actual_pose)
            #*******************************************************************************
            ...
            #*******************************************************************************
            # Finish with the gripper fully open the gripper
            #if self.is_gripper_present:
            #    success &= self.example_send_gripper_command(0.0)
            #else:
            #    rospy.logwarn("No gripper is present on the arm.")  
